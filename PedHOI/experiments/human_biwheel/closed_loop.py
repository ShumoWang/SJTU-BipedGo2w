...
for k in range(steps):
 ...
 correction=np.clip(raw-ff(t),-8.,8.)
 desired=np.clip(ff(t)+correction,m.actuator_ctrlrange[:,0],m.actuator_ctrlrange[:,1])
 d.ctrl[:]=desired if k==0 else np.clip(desired,d.ctrl-1.,d.ctrl+1.)
 ctrl.append(d.ctrl.copy())
