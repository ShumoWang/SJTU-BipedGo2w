...
rot=R.from_quat(q0[:,[4,5,6,3]]);fw=rot.apply(np.tile([0.,0.,-1.],(len(q0),1)));lat=np.cross([0,0,1],fw);frames=np.arange(0,len(q0),8);knots=ts[frames]
sx=[]
for q in q0:
...
fit=least_squares(fun,np.zeros(size),jac_sparsity=sp.tocsr(),bounds=(-.04,.04),max_nfev=35,ftol=1e-8,gtol=1e-9,xtol=1e-8,verbose=1)
q,v,a,delta=candidate(fit.x);after=fun(fit.x)[:size]*weight;metrics=audit(m,q,fps)
np.savez(P/'reference_pre_refinement.npz',**z)
