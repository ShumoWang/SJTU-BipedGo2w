...
height=.65+.22*smooth((tt-lift_start)/(lift_end-lift_start))*(1-smooth((tt-place_start)/(place_end-place_start)))
...
def contacts():
 ...
 for g,v in zip(gids,verts):
  ...
  center=(world.max(0)+world.min(0))/2
  center[2]=world[:,2].max();out.append(center)
 return np.array(out)
errors=[];targets=[]
for k in range(N):
 d.qpos[:]=q[k];bh=R.from_euler('z',byaw[k]);tgt=box[k]+bh.apply(np.array([[0,.190,-size[2]-gap[k]],[0,-.190,-size[2]-gap[k]]]))
 def fun(x):
