...
a=np.load(p.parent/'resmimic_pick_place/go2w_carry_adapted.npz')['qpos'];b=np.load(p/'reference.npz')['qpos'][::4]
c=mujoco.MjvCamera();c.lookat[:]=[-.6,-.3,.5];c.distance=2.7;c.azimuth=235;c.elevation=-15
...
for f in range(0,len(a),3):
 ...
 draw.text((18,5),'Previous: sliding reference',fill='black',font=font)
 draw.text((658,5),'V2: rolling constraints (dynamics FAILED)',fill='black',font=font)
 draw.text((18,29),f'{f/30:.2f} s | '+('PICK / LIFT' if f<85 else 'BLEND IN' if f<100 else 'TRANSPORT' if f<140 else 'BLEND OUT' if f<155 else 'LOWER / PLACE' if f<215 else 'RELEASE / RETRACT')+' | kinematic playback only',fill='#666666',font=font)
 frames.append(np.asarray(out))
 if f==126:out.save(p/'mesh_comparison.png')
