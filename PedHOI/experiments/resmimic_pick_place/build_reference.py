...
Source: ResMimic's already-retargeted G1 kneel.pkl and paired object kneel.npz.
...
s = pickle.load(open(SRC/'kneel.pkl','rb'))
o = np.load(SRC/'kneel.npz')
...
# This clip reaches forward in heading coordinates; no sagittal reflection.
...
feet[:,:,0] -= np.mean(feet[100:140,:,0])
feet[:,:,1] *= 1.8
height = np.clip(.55 + .65*(s['root_pos'][:,2]-np.median(s['root_pos'][100:140,2])), .40, .60)
feet[:,:,2] = .086-height[:,None]+.65*(world[:,[names.index('left_ankle_roll_link'),names.index('right_ankle_roll_link')],2]-.04)
# Hand-selected entry and exit gates from object lift/lower plus hand trajectories.
start,end,exit_start,exit_end = 85,100,140,155
frame=np.arange(n)
def smooth_gate(x):
 x=np.clip(x,0,1)
 return x**3*(10-15*x+6*x*x)
blend=smooth_gate((frame-start)/(end-start))*(1-smooth_gate((frame-exit_start)/(exit_end-exit_start)))
hold_hands = np.median(hands[105:135],axis=0)
...
def solve(targets, heights, tag):
 ...
 for f in range(n):
  ...
  def fun(q):
   ...
   return np.r_[e.ravel(),.03*(q-prev)]
  lo=limits[:,0]+1e-5; hi=limits[:,1]-1e-5
  if f>0:
   lo=np.maximum(lo,prev-.12); hi=np.minimum(hi,prev+.12)
  res=least_squares(fun,seed,bounds=(lo,hi),max_nfev=90,ftol=1e-7)
  seed=res.x; fun(seed)
...
# Integrity, joint limits and maximum inter-frame displacement checks.
assert np.isfinite(aq).all()
assert np.max(np.abs(np.diff(aq[:,7+leg_idx],axis=0)))<=.120001
assert np.all(aq[:,7+leg_idx]>=limits[:,0]-1e-6) and np.all(aq[:,7+leg_idx]<=limits[:,1]+1e-6)
print('max carry leg range',np.ptp(aq[110:140,7+leg_idx],axis=0).max(),flush=True)
summary=dict(source=str(SRC/'kneel.pkl'),paired_object=str(SRC/'kneel.npz'),max_per_frame_leg_change_rad=float(np.abs(np.diff(aq[:,7+leg_idx],axis=0)).max()),source_kind='G1 retargeted motion, not raw human SMPL-X',frames=n,fps=fps,duration_s=(n-1)/fps,phase_transition_frames_zero_based=[start,end,exit_start,exit_end],phase_transition_s=[start/fps,end/fps,exit_start/fps,exit_end/fps],phase_selection='provisional manual selection: pick/lift, transport, lower/place, retract',method='position-only bounded IK prototype; not existing GMR baseline',sagittal_hand_reflection=False,mesh_based_wheel_radius_m=.086,mean_target_error_m=ae.mean(axis=0).tolist(),max_target_error_m=ae.max(axis=0).tolist(),carry_leg_peak_to_peak_rad=float(np.ptp(aq[110:140,7+leg_idx],axis=0).max()),root_xy_path_length_m=float(np.linalg.norm(np.diff(aq[:,:2],axis=0),axis=1).sum()),limitations=['kinematics only; no contact-force, balance, collision or policy validation','source is G1 proxy; morphology transfer cannot recover original human motion','phase-specific mapping and dimensions are heuristic','rear wheel forward rolling approximation; lateral slip not constrained','NPZ is an inspection format, not directly Isaac Lab training input'])
...
def draw(f):
 ...
 for ax,p,edges,title,col in zip(axes,[source_world[f],bp[f],ap[f]],[source_edges,dog_edges,dog_edges],['Source: G1 pick / place','Go2W: framewise position IK','Go2W: phase-adapted'],['#555555','#c68127','#157a8b']):
  for a,b in edges:ax.plot(*p[[a,b]].T,color=col,lw=3)
  ax.scatter(*p.T,color=col,s=6)
  if title.startswith('Source'):
   obj=o['trans'][f]-s['root_pos'][0]*np.array([1,1,0])
   ax.scatter(*obj,color='#a24aad',s=65,marker='s',label='object origin')
  ...
  ax.set_xticks([-1.5,-1,-.5,0]);ax.set_yticks([-1,-.5,0]);ax.set_zticks([0,.4,.8,1.2])
  ax.set(xlim=(-1.9,.4),ylim=(-1.1,.7),zlim=(0,1.35),xlabel='x (m)',ylabel='y (m)',zlabel='z (m)',title=title)
  ax.set_box_aspect((2.3,1.8,1.35));ax.view_init(elev=19,azim=-65)
  xx,yy=np.meshgrid([-1.9,.4],[-1.1,.7]);ax.plot_surface(xx,yy,xx*0,alpha=.08,color='gray')
 phase='PICK / LIFT' if f<start else 'BLEND IN' if f<end else 'TRANSPORT' if f<exit_start else 'BLEND OUT' if f<exit_end else 'LOWER / PLACE' if f<215 else 'RELEASE / RETRACT'
 fig.suptitle(f'ResMimic kneel: pick / carry / place | frame {f:03d} | {f/fps:.2f} s | {phase}\nKinematic prototype only; no physics / grasp validation',fontsize=12)
 fig.tight_layout(rect=[0,0,1,.88]);fig.canvas.draw()
 return np.asarray(fig.canvas.buffer_rgba())[:,:,:3].copy()
frames=[]
for f in range(0,n,3):
 frames.append(draw(f))
 if f==126:fig.savefig(OUT/'comparison.png',dpi=150)
