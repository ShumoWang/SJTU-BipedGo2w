...
for k in range(0,len(tt),8):
 ...
 for leg in ['FL','FR']:
  b=robot.body(leg+'_wheel_link').id;J=np.zeros((3,22));mujoco.mj_jac(robot,rd,J,None,rd.xpos[b]+[0,0,.086],b);lat=R.from_quat(rd.qpos[[4,5,6,3]]).apply([0,1,0]);grip=(-8. if leg=='FL' else 8.)*lat
  rhs+=J.T@(np.array([0,0,.5*9.81/2])+grip)
